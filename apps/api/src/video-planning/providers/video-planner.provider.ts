...
import type {
  ...
  VideoScene,
  VisualDescription,
} from '../interfaces/video-plan.interface';
...
Create the scene plan now.`;
  ...
  private parsePlanResponse(raw: string): VideoPlan {
    try {
      ...
      const scenes = (parsed.scenes ?? []).map((scene, index) => ({
        ...
        durationSeconds:
          scene.durationSeconds ?? this.config.defaultSceneDurationSeconds,
        visual: scene.visual,
      ...
      return {
        ...
        estimatedDurationSeconds:
          ...
          scenes.length * this.config.defaultSceneDurationSeconds,
        scenes,
      };
    } catch (error) {
  ...
  private mockPlan(script: VideoScript): VideoPlan {
    ...
    return {
      title: script.title,
      estimatedDurationSeconds:
        maxScenes * this.config.defaultSceneDurationSeconds,
      scenes,
    };
  }
}
