...
export class ScriptProvider {
    ...
    private buildPrompt(research: ResearchResult): string {
        ...
        const sources = (research.sources || []).map((s) => `- ${JSON.stringify(s)}`).join('\n');
    ...
    private parseScriptResponse(raw: string): VideoScript {
        try {
            ...
            return {
                ...
                fullText: parsed.fullText || [parsed.hook, ...(parsed.sections ?? []), parsed.conclusion].filter(Boolean).join('\n\n'),
                keywords: parsed.keywords ?? [],
                tags: parsed.tags ?? [],
                thumbnailConcept: parsed.thumbnailConcept || '',
                estimatedDurationSeconds: parsed.estimatedDurationSeconds ?? this.config.targetDurationSeconds,
            };
        } catch (error) {
