...
import type { OpenAiCompletionRequest, OpenAiCompletionResponse } from '../interfaces/openai-completion.interface';
...
export class OpenAiClientProvider {
    ...
    async complete(request: OpenAiCompletionRequest): Promise<OpenAiCompletionResponse> {
        ...
        try {
            ...
            this.logger.error(`Unexpected error calling OpenAI API: ${error instanceof Error ? error.message : String(error)}`);
            throw error;
        }
    }
}
