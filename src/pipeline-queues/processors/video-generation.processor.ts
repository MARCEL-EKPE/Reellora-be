...
export class VideoGenerationProcessor extends WorkerHost {
    ...
    async process(job: Job<SceneGenerationJob>): Promise<void> {
        ...
        try {
            ...
            const prompt = typeof scene.visual?.prompt === 'string' ? scene.visual.prompt : `Scene for video ${videoId}`;
            ...
            scene.status = result.status === 'completed' ? SceneStatus.COMPLETED : SceneStatus.FAILED;
            scene.errorMessage = result.errorMessage;
            await this.videoSceneRepository.save(scene);

            await this.checkAndAdvanceToRender(videoId);
        } catch (error) {
            this.logger.error(`[VideoGeneration] failed for sceneId=${sceneId}`, error);
            scene.status = SceneStatus.FAILED;
            scene.errorMessage = error instanceof Error ? error.message : String(error);
            await this.videoSceneRepository.save(scene);
            throw error;
        }
    }
    ...
    private async checkAndAdvanceToRender(videoId: string): Promise<void> {
        ...
        const allDone = scenes.every((s) => s.status === SceneStatus.COMPLETED || s.status === SceneStatus.FAILED);

        if (allDone) {
            this.logger.log(`[VideoGeneration] all scenes processed for videoId=${videoId}; advancing to render`);
