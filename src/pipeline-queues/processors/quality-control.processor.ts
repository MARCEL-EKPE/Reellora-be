...
export class QualityControlProcessor extends WorkerHost {
    ...
    async process(job: Job<PipelineJob>): Promise<void> {
        ...
        try {
            ...
            const finalOutputPath = finalAsset?.storageKey || `videos/${videoId}/final.mp4`;
            const result = await this.qualityControlProvider.validateVideo(videoId, finalOutputPath);
            ...
            this.logger.error(`[QualityControl] failed for videoId=${videoId}`, error);
